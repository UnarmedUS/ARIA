...
# -*- coding: utf-8 -*-
"""
ARIA (Adaptive, Responsive, Intelligent Assistant) — minimal, production-safe starter
- Framework: discord.py v2 (slash commands via app_commands)
- Env var: DISCORD_TOKEN (set in Railway/Render/Heroku etc.)
- Data storage: local JSON (data/users.json, data/servers.json, data/logs.json)
...
intents.guilds = True
intents.members = True          # enable "Server Members Intent" in Discord Dev Portal
intents.message_content = True  # required for mention awareness and tips
    ...
    if bot.user in message.mentions:
        try:
            await message.channel.send(
                "👋 I’m here! Try `/help` for commands, or `/introduce` to see what I can do in this server."
            )
        except Exception:
            pass
...
@bot.tree.command(name="about", description="Learn more about ARIA.")
async def about(interaction: discord.Interaction):
    msg = (
        "**🤖 ARIA** — Adaptive, Responsive, Intelligent Assistant\n"
        "Purpose: coaching, coordination, and healthy community interactions.\n"
        "Status: Core online."
    )
...
async def help_cmd(interaction: discord.Interaction):
    commands_list = [
        ...
        "`/help` — This list",
        "`/introduce` — Context-aware intro",
        "`/profile [field] [value]` — Personal options",
